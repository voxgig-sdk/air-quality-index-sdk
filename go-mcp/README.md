# air-quality-index-mcp

[MCP](https://modelcontextprotocol.io) server exposing the AirQualityIndex SDK as
two agent tools — `air-quality-index_list` and `air-quality-index_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/air-quality-index-mcp)
make build

# 2. Provide credentials via the environment
export AIR_QUALITY_INDEX_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user air-quality-index \
  -- /absolute/path/to/air-quality-index-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./air-quality-index-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// air-quality-index_list: first page of records
{ "entity": "aqi" }
{ "entity": "aqi", "query": { } }

// air-quality-index_load: one record by id
{ "entity": "aqi", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/air-quality-index-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user air-quality-index \
     -- "$PWD"/dist/*/air-quality-index-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `air-quality-index_list` and `air-quality-index_load` tools now appear
   in new sessions. Ask the agent to *"list aqi using air-quality-index"*
   and it calls `air-quality-index_list` with `{"entity":"aqi"}`.
...
```sh
export AIR_QUALITY_INDEX_APIKEY=sk_live_xxx            # API key
export AIR_QUALITY_INDEX_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `air-quality-index_list` tool
...
### Call the `air-quality-index_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `air-quality-index_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `air-quality-index_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AIR_QUALITY_INDEX_APIKEY` | API key sent with every request. |
| `AIR_QUALITY_INDEX_BASE` | Optional override of the API base URL. |
...
```sh
./air-quality-index-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"air-quality-index_load","arguments":{"entity":"aqi","query":{"id":1}}}}'
```
