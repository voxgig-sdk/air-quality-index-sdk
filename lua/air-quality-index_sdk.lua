...
require("air-quality-index_types")
...
local AirQualityIndexSDK = {}
AirQualityIndexSDK.__index = AirQualityIndexSDK
...
AirQualityIndexSDK._make_feature = _make_feature


function AirQualityIndexSDK.new(options)
  local self = setmetatable({}, AirQualityIndexSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AirQualityIndexSDK:options_map()
...
function AirQualityIndexSDK:get_utility()
  return Utility.copy(self._utility)
end


function AirQualityIndexSDK:get_root_ctx()
  return self._rootctx
end


function AirQualityIndexSDK:prepare(fetchargs)
...
function AirQualityIndexSDK:direct(fetchargs)
...
function AirQualityIndexSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function AirQualityIndexSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "AirQualityIndexSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function AirQualityIndexSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AirQualityIndexSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AirQualityIndexSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AirQualityIndexSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AirQualityIndexSDK.new(sdkopts)
  sdk.mode = "test"
