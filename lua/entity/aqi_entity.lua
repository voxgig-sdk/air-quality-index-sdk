...
local AqiEntity = {}
AqiEntity.__index = AqiEntity


function AqiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AqiEntity)
  self._name = "aqi"
  self._client = client
...
function AqiEntity:get_name()
  return self._name
end


function AqiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AqiEntity.new(self._client, opts)
end
...
function AqiEntity:mark_deleted()
  self._deleted = true
end


function AqiEntity:deleted()
  return true == self._deleted
end


function AqiEntity:data_set(args)
  if args ~= nil then
...
function AqiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AqiEntity:match_set(args)
  if args ~= nil then
...
function AqiEntity:stream(action, args, callopts)
...
---@param reqmatch AqiLoadMatch
---@param ctrl? table
---@return Aqi
---@return string? err
function AqiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AqiEntity:_run_op(ctx, post_done)
  local utility = self._utility
