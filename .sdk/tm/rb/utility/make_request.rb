...
module AirQualityIndexUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AirQualityIndexResponse.new({})
    ...
    if fetch_err
      ...
      response = AirQualityIndexResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AirQualityIndexResponse.new(fetched)
    else
