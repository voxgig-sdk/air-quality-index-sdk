...
// AirQualityIndex SDK feature test
//
...
class FtCtrl extends AirQualityIndexControl
{
...
class FtClient
{
    ...
    private AirQualityIndexUtility $utility;

    public function __construct(AirQualityIndexUtility $utility, array $options)
    {
    ...
    public function get_utility(): AirQualityIndexUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AirQualityIndexContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AirQualityIndexContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AirQualityIndexContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AirQualityIndexConfig::make_feature($name);
    ...
    private static function buildUrl(AirQualityIndexSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AirQualityIndexError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AirQualityIndexResponse($fetched);
        }
    ...
    private function fail(AirQualityIndexContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AirQualityIndexContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
