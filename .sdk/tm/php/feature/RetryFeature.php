...
// AirQualityIndex SDK retry feature
...
class AirQualityIndexRetryFeature extends AirQualityIndexBaseFeature
{
    ...
    public function init(AirQualityIndexContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AirQualityIndexContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AirQualityIndexContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AirQualityIndexContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
