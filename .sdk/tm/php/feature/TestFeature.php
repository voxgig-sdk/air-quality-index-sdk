...
// AirQualityIndex SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AirQualityIndexTestFeature extends AirQualityIndexBaseFeature
{
    ...
    public function init(AirQualityIndexContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AirQualityIndexContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AirQualityIndexParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AirQualityIndexContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AirQualityIndexContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AirQualityIndexParam::call($ctx, $k);
