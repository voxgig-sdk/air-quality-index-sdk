# AirQualityIndex SDK test runner
...
class AirQualityIndexTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AirQualityIndexTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AirQualityIndexTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AirQualityIndexTestRunner.getenv("AIR_QUALITY_INDEX_TEST_LIVE")
        override = AirQualityIndexTestRunner.getenv("AIR_QUALITY_INDEX_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AirQualityIndexTestRunner.getenv(key)
        ...
        explain = AirQualityIndexTestRunner.getenv("AIR_QUALITY_INDEX_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["AIR_QUALITY_INDEX_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AirQualityIndexTestRunner._test_control is not None:
            return AirQualityIndexTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AirQualityIndexTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AirQualityIndexTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AirQualityIndexTestRunner.load_test_control()
...
def load_env_local():
    AirQualityIndexTestRunner.load_env_local()


def env_override(m):
    return AirQualityIndexTestRunner.env_override(m)


def entity_data(v):
    return AirQualityIndexTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AirQualityIndexTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AirQualityIndexTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AirQualityIndexTestRunner.load_test_control()


def live_client_options():
    return AirQualityIndexTestRunner.live_client_options()


def live_delay_ms():
    return AirQualityIndexTestRunner.live_delay_ms()
