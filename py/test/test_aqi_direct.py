...
class TestAqiDirect:
    ...
    def test_should_direct_load_aqi(self):
        setup = _aqi_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-aqi", "live" if setup["live"] else "unit")
        if _skip:
...
def _aqi_direct_setup(mockres):
    ...
    live = env.get("AIR_QUALITY_INDEX_TEST_LIVE") == "TRUE"
