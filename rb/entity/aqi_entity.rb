...
class AqiEntity
  ...
  def make
    opts = @_entopts.dup
    AqiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AirQualityIndexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Aqi, Hash] the current Aqi data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AirQualityIndexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Aqi fields)
  def match_get
  ...
  # Load a single Aqi.
  #
  # @param reqmatch [AqiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Aqi.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Aqi, Hash] the loaded Aqi; raises AirQualityIndexError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AirQualityIndexHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
