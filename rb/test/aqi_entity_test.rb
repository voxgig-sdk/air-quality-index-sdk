...
class AqiEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = aqi_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "aqi." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    aqi_ref01_data = nil
    if aqi_ref01_data_raw.length > 0
...
def aqi_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "aqi", "AqiTestData.json")
