...
require_relative 'AirQualityIndex_types'
...
class AirQualityIndexSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AirQualityIndexHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AirQualityIndexHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AirQualityIndexFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AirQualityIndexUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AirQualityIndexHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AirQualityIndexHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AirQualityIndexHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AirQualityIndexError => err
    ...
    if fetched.is_a?(Hash)
      status = AirQualityIndexHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AirQualityIndexError.new(
        "graphql_error", "AirQualityIndexSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AirQualityIndexSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
