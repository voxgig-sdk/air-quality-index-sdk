...
boru-driven command-line client **and** interactive REPL for the AirQualityIndex
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/air-quality-index-cli)
make build

# 2. See usage (words, entities, env vars)
./air-quality-index-cli --help

# 3. Provide credentials once, via the environment
export AIR_QUALITY_INDEX_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./air-quality-index-cli load 1 aqi            # {id:1} shorthand
./air-quality-index-cli load '{id:1}' aqi       # explicit match map

# 5. Override the API base URL for a single call
AIR_QUALITY_INDEX_BASE=https://api.example.com ./air-quality-index-cli load 1 aqi

# 6. No arguments -> interactive REPL
./air-quality-index-cli
air-quality-index> load 1 aqi
air-quality-index> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/air-quality-index-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/air-quality-index-cli load 1 aqi
   ```
...
```sh
./air-quality-index-cli load 1 aqi          # scalar shorthand for {id:1}
./air-quality-index-cli load '{id:1}' aqi     # explicit match map
```
...
./air-quality-index-cli load 1 aqi
```
...
Run with no arguments to open a REPL (prompt `air-quality-index>`). Each line is
evaluated as its own boru expression:

```text
$ ./air-quality-index-cli
air-quality-index> load 1 aqi
air-quality-index> /help
air-quality-index> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `aqi`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/air-quality-index-cli`. |
...
(`./air-quality-index-cli <expr>`) and interactively (the REPL), and expressions compose the
