...
import (
	...
	omni "github.com/voxgig-sdk/air-quality-index-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AirQualityIndexError); is {
				out := map[string]any{
					"name":    "AirQualityIndexError",
...
func omniCtx(arg any, client *sdk.AirQualityIndexSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AirQualityIndexSDK); is {
			client = live
