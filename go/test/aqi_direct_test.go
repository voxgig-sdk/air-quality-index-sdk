...
func TestAqiDirect(t *testing.T) {
	t.Run("direct-load-aqi", func(t *testing.T) {
		setup := aqiDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-aqi", _mode); _shouldSkip {
			if _reason == "" {
...
type aqiDirectSetupResult struct {
...
func aqiDirectSetup(mockres any) *aqiDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &aqiDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &aqiDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
