...
The Air Quality Index (AQI) API provides real-time air quality data, including the AQI and specific pollutant concentrations (such as PM2.5, PM10, CO, NO2, SO2, and O3) for various locations around the world.

Learn more about Air Quality Index API at [www.juheapi.com/api-catalog/aqi](https://www.juheapi.com/api-catalog/aqi).
...
This SDK exposes the API as a small set of **semantic entities** — Aqi — that you
...
const aqi = await client.Aqi().load()
// aqi is the Aqi entity, populated with mock data
...
// Load a specific aqi (returns the ENTITY; call data_get() for the record; throws on error)
$aqi = $client->Aqi()->load();
...
# Load a specific aqi (returns the ENTITY; call data_get for the record)
aqi = client.Aqi.load()
puts aqi
```
