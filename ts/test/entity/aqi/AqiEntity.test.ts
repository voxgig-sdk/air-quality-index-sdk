...
describe('AqiEntity', async () => {
  ...
  // `test.live.delayMs`; only sleeps when AIRQUALITYINDEX_TEST_LIVE=TRUE.
  afterEach(liveDelay('AIRQUALITYINDEX_TEST_LIVE'))
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'aqi.' + op, live)) return
    }
    ...
    const aqi_ref01_data_dt0 = await aqi_ref01_ent.load(aqi_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['AIR_QUALITY_INDEX_TEST_AQI_ENTID']
