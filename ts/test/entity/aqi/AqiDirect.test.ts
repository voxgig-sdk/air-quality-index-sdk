...
describe('AqiDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AirQualityIndexSDK({
  ...
  test('direct-load-aqi', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-aqi', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AIR_QUALITY_INDEX_TEST_AQI_ENTID']
