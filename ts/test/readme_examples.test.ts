...
//     (\`new AirQualityIndexSDK(...)\` / \`AirQualityIndexSDK.test(...)\` become
//     \`AirQualityIndexSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AirQualityIndexSDK'
...
const TEST_SEED = {"entity":{"aqi":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AirQualityIndexSDK(...)\` and any
// \`AirQualityIndexSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AirQualityIndexSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AirQualityIndexSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
