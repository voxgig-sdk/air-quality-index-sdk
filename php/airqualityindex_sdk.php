...
class AirQualityIndexSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AirQualityIndexHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AirQualityIndexHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AirQualityIndexFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AirQualityIndexFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AirQualityIndexUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AirQualityIndexHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AirQualityIndexHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AirQualityIndexHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AirQualityIndexError($op . "_allow",
                "AirQualityIndexSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AirQualityIndexHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AirQualityIndexError("graphql_error",
                "AirQualityIndexSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Aqi($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_aqi === null) {
                $this->_aqi = new AqiEntity($this, null);
            }
            return $this->_aqi;
        }
        return new AqiEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new AirQualityIndexSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
