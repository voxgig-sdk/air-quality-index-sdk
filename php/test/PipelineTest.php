...
// AirQualityIndex SDK pipeline test
//
...
{
    private static function utility(): AirQualityIndexUtility
    {
        return new AirQualityIndexUtility();
    }
    ...
    private static function ctx(array $over = []): AirQualityIndexContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AirQualityIndexContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AirQualityIndexOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AirQualityIndexError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AirQualityIndexOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AirQualityIndexMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AirQualityIndexOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AirQualityIndexMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AirQualityIndexOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AirQualityIndexMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AirQualityIndexSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AirQualityIndexMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AirQualityIndexResponse([]),
            'result' => new AirQualityIndexResult([]),
        ]);
        [, $err] = AirQualityIndexMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AirQualityIndexSpec([]),
            'response' => null,
            'result' => new AirQualityIndexResult([]),
        ]);
        [, $err] = AirQualityIndexMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AirQualityIndexSpec(['step' => 's']),
            'response' => new AirQualityIndexResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AirQualityIndexResult([]),
        ]);
        [, $err] = AirQualityIndexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AirQualityIndexSpec(['step' => 's']),
            'response' => new AirQualityIndexResponse(self::resp(200, ['v' => 1])),
            'result' => new AirQualityIndexResult([]),
        ]);
        [, $err] = AirQualityIndexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AirQualityIndexSpec(['step' => 's']),
            'response' => new AirQualityIndexResponse(self::resp(200, ['v' => 2])),
            'result' => new AirQualityIndexResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AirQualityIndexMakeResponse::call($ctx);
    ...
    {
        $preset = new AirQualityIndexResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AirQualityIndexSpec([]),
            'response' => new AirQualityIndexResponse([]),
            'result' => new AirQualityIndexResult([]),
        ...
        [$got, $err] = AirQualityIndexMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AirQualityIndexResult([])]);
        [, $err] = AirQualityIndexMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AirQualityIndexSpec([]), 'result' => null]);
        [, $err] = AirQualityIndexMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AirQualityIndexSpec(['step' => 's']),
            'result' => new AirQualityIndexResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AirQualityIndexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AirQualityIndexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AirQualityIndexSpec(['step' => 's']),
            'result' => new AirQualityIndexResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AirQualityIndexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AirQualityIndexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AirQualityIndexResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AirQualityIndexSpec([]),
            'result' => new AirQualityIndexResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AirQualityIndexMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AirQualityIndexMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AirQualityIndexError('boom', 'boom');
        $utility->fetcher = function (AirQualityIndexContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AirQualityIndexSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AirQualityIndexMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AirQualityIndexContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AirQualityIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AirQualityIndexResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AirQualityIndexMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AirQualityIndexContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AirQualityIndexSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AirQualityIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AirQualityIndexResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AirQualityIndexSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AirQualityIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AirQualityIndexMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AirQualityIndexMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AirQualityIndexResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AirQualityIndexDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AirQualityIndexResult(['ok' => false])]);
        $this->expectException(AirQualityIndexError::class);
        AirQualityIndexDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AirQualityIndexResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AirQualityIndexMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AirQualityIndexMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AirQualityIndexBaseFeature();
        $b = new AirQualityIndexBaseFeature();
        AirQualityIndexFeatureAdd::call($ctx, $a);
        AirQualityIndexFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AirQualityIndexBaseFeature
    {
        $f = new AirQualityIndexBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AirQualityIndexFeatureAdd::call($ctx, self::named_feature('a'));
        AirQualityIndexFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        AirQualityIndexFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        AirQualityIndexFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        AirQualityIndexFeatureAdd::call($ctx, $replace);
        ...
        AirQualityIndexFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AirQualityIndexContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AirQualityIndexSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AirQualityIndexPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AirQualityIndexPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AirQualityIndexResponse(['headers' => null]),
            'result' => new AirQualityIndexResult([]),
        ]);
        AirQualityIndexResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        AirQualityIndexResultBody::call($ctx);
